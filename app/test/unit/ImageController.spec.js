...
describe('Image Controller', function () {
    ...
    it('should has an image selected', function () {
        scope.keyword = "";
...
describe('Image Controller FileSystems', function () {
    ...
    beforeEach(inject(function ($controller, $rootScope, $q) {
        ...
        var fakeGoogleImageService = getFakeErrorImageGoogleService($q, $rootScope);
        var fakeOpenDialog = getFakeOpenDialog();
        var fakeFS = getFakeFileSystem();
        ctrl = $controller('ImageController', {$scope: scope, GoogleImageService: fakeGoogleImageService,
                            DummyGoogleImageService: fakeGoogleImageService,
                            ElectronOpenFileDialogFactory: fakeOpenDialog,
                            ElectronFileSystemFactory: fakeFS,
        });
        rootScope = $rootScope;
    ...
    it('should when imageDropped return encoded image', function () {
        setCardInitData();
        scope.imageDropped();
    ...
    it('should when selectDialog saveImage', function (done) {
        setCardInitData(null);
        scope.selectDialog();
        setTimeout(function () {
            rootScope.$apply();
            done();
...
function getFakeOpenDialog() {
    var FakeOpenDialog = function () {
        
    };
    var FakeDialog = function () {

    };
    FakeDialog.prototype.showOpenDialog = function (callback) {
        callback('test');
    };
    FakeOpenDialog.prototype.getOpenFileDialog = function () {
        return new FakeDialog();
    };
    return new  FakeOpenDialog();
}

function getFakeFileSystem() {
    var FakeFS = function () {

    };
    FakeFS.prototype.getBase64FileSystemEncoder = function(filename){
        var fileS = new getFakeGoodFileSystem( 'test', 'tests');
